...
interface CommunityRequest {
  ...
  urgency: string;
  description: string;
  initiative: string; // Assuming 'community-request' or similar
  submittedAt: string;
  source: string;
}

const CommunityDonorSurakshaSetu: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-gray-800 rounded-xl p-8 mt-8 shadow-lg">
      <h2 className="text-2xl font-bold text-blue-400 mb-6">🤝 Suraksha Setu - Community Donor Response</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-gray-400 text-sm">Needed Item Types</label>
              <div className="flex flex-wrap gap-2">
                {request.neededItemTypes && request.neededItemTypes.length > 0 ? request.neededItemTypes.map((item) => (
                  <span key={item} className="bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-medium">{item}</span>
                )) : <span className="text-gray-400">None</span>}
              </div>
            </div>
            <div>
              <label className="block text-gray-400 text-sm">Quantity Required</label>
              <input className="w-full bg-gray-700 rounded px-3 py-2 text-white" value={request.quantityRequired} readOnly />
            </div>
          </div>
        </div>
        {/* Donor Information */}
...
export default CommunityDonorSurakshaSetu; 